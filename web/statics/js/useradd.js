...
$(function(){
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:path+"/user/isExists.html",//请求的url
			data:{userCode:userCode.val()},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(data.result == "false"){//账号已存在，错误提示
					validateTip(userCode.next(),{"color":"red"},imgNo+ data.message,false);
				}else{//账号可用，正确提示
					validateTip(userCode.next(),{"color":"green"},imgYes+data.message,true);
				}
			},
			error:function(data){//当访问时候，404，500 等非200的错误状态码
