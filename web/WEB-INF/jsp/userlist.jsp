...
<%@include file="/WEB-INF/jsp/common/head.jsp"%>
        <div class="right">
            ...
            <div class="search">
           		<form method="post" action="${pageContext.request.contextPath }/user/list.html">
					<input name="method" value="query" class="input-text" type="hidden">
					 <span>用户名：</span>
					 <input name="userName" class="input-text"	type="text" value="${param.userName }">
					 
					 <span>用户角色：</span>
					 <select name="userRole">
						 <option value="0">--请选择--</option>
						 <option value="1"
							 <c:if test="${param.userRole eq 1}">
								 selected
							 </c:if>
						 >系统管理员</option>
						 <option value="2"
							 <c:if test="${param.userRole eq 2}">
								 selected
							 </c:if>
						 >经理</option>
						 <option value="3"
							 <c:if test="${param.userRole eq 3}">
								 selected
							 </c:if>
						 >普通员工</option>
			...
			<input type="hidden" id="totalPageCount" value="${pageSupport.pageCount}"/>
		  	<c:import url="rollpage.jsp">
	          	<c:param name="totalCount" value="${pageSupport.totalCount}"/>
	          	<c:param name="currentPageNo" value="${pageSupport.pageIndex}"/>
	          	<c:param name="totalPageCount" value="${pageSupport.pageCount}"/>
          	</c:import>
